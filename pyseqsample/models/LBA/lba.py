...
class LBAAccumulator(Accumulator):
    ...
    def pdf(self, t, condition=0, **kwargs):

        if self.params.shape[0] == 1:
            return pdf(t=t, *self.params[0, :].tolist(), **kwargs)
        ...
        for cond in np.arange(self.n_conditions):
            idx = condition == cond
            density[idx] =  pdf(t=t[idx], *self.params[cond, :].tolist(), **kwargs)

        return density
    
    def cdf(self, t, condition=0, **kwargs):

        if self.params.shape[0] == 1:
            return cdf(t=t, *self.params[0, :].tolist(), **kwargs)
        ...
        for cond in np.arange(self.n_conditions):
            idx = condition == cond
            density[idx] =  cdf(t=t[idx], *self.params[cond, :].tolist(), **kwargs)
...
def pdf(ter, A, v, sv, b, t, robust=False, posdrift=True):
    ...
    if posdrift:
        denom = np.max((pnorm1(v/sv), 1e-10))
    else:
        denom = 1
    
    t=np.maximum(t-ter, 1e-5) # absorbed into pdf 
    if A<1e-10: # LATER solution
        return np.maximum(1e-10, (b/(t**2)*dnorm1(b/t, mean=v,sd=sv)) /denom )
    zs=t*sv
    zu=t*v
    bminuszu=b-zu
    bzu=bminuszu/zs
    ...
    return np.maximum(1e-10, ((v*(pnorm1(bzu)-pnorm1(bzumax)) +
                    sv*(dnorm1(bzumax)-dnorm1(bzu)))/A)/denom)


def cdf(ter, A, v, sv, b, t, robust=False, posdrift=True):
    ...
    return np.minimum(np.maximum(1e-10,(1+(tmp1+tmp2)/A)/denom), 1 - 1e-10)
